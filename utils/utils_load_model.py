...
def model_equal_part(model, dict_all):
    model_dict = model.state_dict()
    dict_fix = {k: v for k, v in dict_all.items() if k in model_dict and k.find('embedding') == -1 and k.find('fc') == -1 and k.find('adv_layer') == -1}
...
def load_model_norm(model):
    ...
    for k, v in dict_all.items():
        ...
        idt = k.find('conv_2.weight')
        if idt >= 0:
            w_mu = v.mean([2, 3], keepdim=True)
            w_std = v.std([2, 3], keepdim=True) * stdd
            dict_all[k].data = (v - w_mu)/(w_std)
            dict_all[k[:idt] + 'AdaFM_2.style_gama'].data = w_std.data
            dict_all[k[:idt] + 'AdaFM_2.style_beta'].data = w_mu.data
